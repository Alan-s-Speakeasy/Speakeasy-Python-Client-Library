...
## Development
This pacakge `speackeasypy` depends on an internal package `speakeasypy.openapi.client` which is generated by openapi. 
Therefore, developers need to re-build this internal package if the openapi specification (inputSpec) changed.

Install `speackeasypy` locally:
```shell
python setup.py install
````
Distribute `speackeasypy` and test it locally:
```shell
